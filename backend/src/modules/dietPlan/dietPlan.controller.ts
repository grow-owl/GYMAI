...
export class DietPlanController {
  ...
  public static getActiveDietPlan = asyncHandler(async (req: Request, res: Response) => {
    ...
    const gymId = req.user?.gymId?.toString() || req.params.gymId;
    const dietPlan = await DietPlanService.getActiveDietPlanForMember(memberId, gymId);
  ...
  public static listDietPlans = asyncHandler(async (req: Request, res: Response) => {
    const memberId = req.params.memberId || req.user!.id;
    const gymId = req.user?.gymId?.toString() || req.params.gymId;
    const { plans, meta } = await DietPlanService.listDietPlans(memberId, req.query, gymId);
